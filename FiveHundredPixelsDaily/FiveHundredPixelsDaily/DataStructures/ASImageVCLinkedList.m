...
- (ASImageVCLinkedList *)prev {
    ...
    if (self.prevExists == true && _prev != nil) {
        ...
        NSUInteger imageIndex = [self.imageVC.image.category.mildImages indexOfObject:self.imageVC.image];
        if (imageIndex != 0) {
            ...
            newImageVC.image = self.imageVC.image.category.mildImages[imageIndex-1];
...
- (ASImageVCLinkedList *)next {
    ...
    if (self.nextExists == true && _next != nil) {
        ...
        if (imageIndex != self.imageVC.image.category.mildImages.count-1) {
            ASImageViewController *newImageVC = [[UIStoryboard storyboardWithName:@"Main" bundle:[NSBundle mainBundle]] instantiateViewControllerWithIdentifier:@"FullImageVC"];
            newImageVC.image = self.imageVC.image.category.mildImages[imageIndex+1];
            ...
            // If it was the last image, we need to make sure it is because the category is not going to get anymore new mildImages
